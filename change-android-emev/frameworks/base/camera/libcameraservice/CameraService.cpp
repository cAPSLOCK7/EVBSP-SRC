...
status_t CameraService::Client::setOverlay()
{
    ...
    if (!strcmp(format, "yuv422sp")){
        fmt = OVERLAY_FORMAT_YCbCr_422_SP;
    }
    else if (!strcmp(format, "yuv420p")){
        fmt = OVERLAY_FORMAT_YCbCr_420_P;
    }
    else if (!strcmp(format, "yuv420sp")){
        fmt = OVERLAY_FORMAT_YCbCr_420_SP;
    }
    else {
...
//                mOverlayRef = mSurface->createOverlay(w, h, OVERLAY_FORMAT_DEFAULT,  mOrientation);
                mOverlayRef = mSurface->createOverlay(w, h, fmt,mOrientation);
    //            LOGD("[ ++++++++= debug++++] %s:%d,w,h,fmt=0x%x,fmt=%d",__FUNCTION__,__LINE__,w,h,fmt,fmt);
