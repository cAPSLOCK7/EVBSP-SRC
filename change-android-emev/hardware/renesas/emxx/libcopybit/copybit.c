...
const struct copybit_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "Renesas EMXX COPYBIT Module",
        .author = "Renesas, Corp.",
...
/** convert COPYBIT_FORMAT to EMXX format */
static int get_format(int format) {
    switch (format) {
    case COPYBIT_FORMAT_RGB_888:	return EMXX_FB_BGR_888;
    case COPYBIT_FORMAT_RGB_565:	return EMXX_FB_RGB_565;
    case COPYBIT_FORMAT_RGBA_8888:	return EMXX_FB_ABGR_8888;
    case COPYBIT_FORMAT_BGRA_8888:	return EMXX_FB_ARGB_8888;
    }
    return -1;
}

/** convert from copybit image to emxx image structure */
static void set_image(struct emxx_fb_img *img,
...
/** setup emxx request */
static void set_infos(struct copybit_context_t *dev, struct emxx_fb_blit_req *req) {
    req->alpha = dev->mAlpha;
    req->transp_mask = EMXX_FB_TRANSP_NOP;
    req->flags = dev->mFlags;
}

/** copy the bits */
static int emxx_copybit(struct copybit_context_t *dev, void const *list) 
{
    int err = ioctl(dev->mFD, EMXX_FB_BLIT,
                    (struct emxx_fb_blit_req_list const*)list);
        ...
        int value) 
{
    ...
    if (ctx) {
        ...
        case COPYBIT_ROTATION_DEG:
            ...
            case 90:
                ctx->mFlags &= ~0x7;
                ctx->mFlags |= EMXX_FB_ROT_90;
                break;
            case 180:
                ctx->mFlags &= ~0x7;
                ctx->mFlags |= EMXX_FB_ROT_180;
                break;
            case 270:
                ctx->mFlags &= ~0x7;
                ctx->mFlags |= EMXX_FB_ROT_270;
                break;
            default:
        ...
        struct copybit_region_t const *region) 
{
    ...
    if (ctx) {
        struct {
            uint32_t count;
            struct emxx_fb_blit_req req[12];
        } list;

        switch (src->format) {
            // we don't support these formats
        ...
        struct hw_device_t** device)
{
    ...
    ctx->mAlpha = EMXX_FB_ALPHA_NOP;
    ctx->mFlags = 0;
    ctx->mFD = open("/dev/graphics/fb0", O_RDWR, 0);
    
    if (ctx->mFD < 0) {
        status = errno;
        LOGE("Error opening fb errno=%d (%s)",
        ...
        if (ioctl(ctx->mFD, FBIOGET_FSCREENINFO, &finfo) == 0) {
            if (strcmp(finfo.id, "EMXX FB") == 0) {
                status = emxx_copybit(ctx, &list);
            } else {
                LOGE("Error not emxx frame buffer");
                status = -EINVAL;
            }
        } else {
