...
static void
emxx_copy_buffer(buffer_handle_t handle, int fd, int width, int height,
...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        emxx_copy_buffer(m->framebuffer, m->framebuffer->fd,
                ...
                int x, int y, int w, int h)
{
    ...
    blit.req.flags = 0;
    blit.req.alpha = 0xff;
    blit.req.transp_mask = EMXX_FB_TRANSP_NOP;
    ...
    if (fb_bits_per_pixel == 24)
        blit.req.dst.format = EMXX_FB_BGR_888;
    else
        blit.req.dst.format = EMXX_FB_RGB_565;
    ...
    if (ioctl(fd, EMXX_FB_BLIT, &blit))
        LOGE("EMXX_FB_BLIT failed = %d", -errno);
}
