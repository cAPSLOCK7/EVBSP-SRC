...
# u-boot-armdev ARM Supplied Development Boards branch details
ARMLTD_BRANCH = 090204_armdev
ARMLTD_RELEASE = AEL-3.0
ARMLTDVC_FILE = $(obj)include/armltdvc_autogenerated.h
...
U_BOOT_EMMC = $(obj)u-boot-emmc.bin
endif

ifeq ($(CONFIG_ESD_U_BOOT),y)
U_BOOT_ESD = $(obj)u-boot-esd.bin
endif

ifeq ($(CONFIG_SD_U_BOOT),y)
U_BOOT_SD = $(obj)uboot-sd.bin
MINI_BOOT_SD = $(obj)sdboot.bin
endif

ifeq ($(CONFIG_IHEX_U_BOOT),y)
U_BOOT_IHEX = $(obj)u-boot.ihex
endif
...
ALL += $(U_BOOT_EMMC) $(U_BOOT_ESD) $(U_BOOT_SD) $(MINI_BOOT_SD) $(U_BOOT_IHEX)
...
$(U_BOOT_EMMC):	$(obj)u-boot.bin $(obj)include/autoconf.mk
		$(MAKE) -C board/$(BOARDDIR)/mini-boot/mmc

$(U_BOOT_ESD):	$(obj)u-boot.bin $(obj)include/autoconf.mk
		$(MAKE) -C board/$(BOARDDIR)/mini-boot/mmc

$(U_BOOT_SD):	$(obj)u-boot.bin $(obj)include/autoconf.mk
		cat $(obj)u-boot.bin > $(obj)uboot-sd.bin

$(MINI_BOOT_SD): $(obj)include/autoconf.mk
		$(MAKE) -C board/$(BOARDDIR)/mini-boot/mmc

$(U_BOOT_IHEX): $(obj)u-boot.bin
		$(OBJCOPY) -I binary -O ihex --adjust-vma=0 u-boot.bin $@

$(VERSION_FILE):
		@( printf '#define U_BOOT_VERSION "U-Boot %s%s"\n' "$(U_BOOT_VERSION)" \
		 '$(shell $(TOPDIR)/tools/setlocalversion $(TOPDIR))' ) > $@.tmp
		@cmp -s $@ $@.tmp && rm -f $@.tmp || mv -f $@.tmp $@
		@echo -n "#define ARMLTD_BRANCH        \""	>  $(ARMLTDVC_FILE) 
		@echo "$(ARMLTD_BRANCH)\""			>> $(ARMLTDVC_FILE) 
		@echo -n "#define ARMLTD_RELEASE       \""	>> $(ARMLTDVC_FILE) 
		@echo "$(ARMLTD_RELEASE)\""			>> $(ARMLTDVC_FILE) 
		@if [ -d .git ] ; then										\
			echo -n "#define ARMLTD_COMMIT_REMOTE \"" 	>>  $(ARMLTDVC_FILE) ;			\
			text=`cat .git/refs/remotes/origin/$(ARMLTD_BRANCH)` ;					\
			echo -n $$text					>>  $(ARMLTDVC_FILE) ;		    	\
			echo "\""					>>  $(ARMLTDVC_FILE) ;		    	\
			echo -n "#define ARMLTD_COMMIT_LOCAL  \""	>>  $(ARMLTDVC_FILE) ;		    	\
			text=`cat .git/refs/heads/$(ARMLTD_BRANCH)` ;						\
			echo -n $$text					>>  $(ARMLTDVC_FILE) ;		    	\
			echo "\""					>>  $(ARMLTDVC_FILE) ;		    	\
			text=`git status`;									\
			echo "$$text" ;										\
			echo -n "#define ARMLTD_SUPPLEMENTARY \" "		>>  $(ARMLTDVC_FILE) ;		\
			echo $$text | grep "nothing to commit"  >& /dev/null ;					\
			if [ $$? -eq 0 ] ; then						     			\
				echo "No uncommitted changes\""		>>  $(ARMLTDVC_FILE) ;			\
			else											\
				echo "Uncommitted changes\""		>>  $(ARMLTDVC_FILE) ;			\
			fi;											\
		else												\
			echo "#define ARMLTD_COMMIT_REMOTE \"UNKNOWN\""		>>  $(ARMLTDVC_FILE)	     ;	\
			echo "#define ARMLTD_COMMIT_LOCAL  \"UNKNOWN\"" 	>>  $(ARMLTDVC_FILE)	     ;	\
			echo "#define ARMLTD_SUPPLEMENTARY \"UNKNOWN\""  	>>  $(ARMLTDVC_FILE)	     ;	\
		fi;												\
...
#
# Note the kludge to prevent failing on the first clean make
# - I wanted to stick with one armsupplied.h, rather than add armltd_version.h
#
...
#========================================================================
# ARM supplied RealView boards
# - detected using HBI
realview_config :	unconfig
	@$(MKCONFIG) $(@:_config=) arm armct_rv realview armltd

# ARM supplied RealView Emulation Baseboard
realview_eb_config :	unconfig
	@chmod a+x board/armltd/realview_eb/split_by_variant.sh 
	@board/armltd/realview_eb/split_by_variant.sh $@ $(CC)
...
emev_pdium_emmc_config : unconfig
	...
	elif [ "$(findstring _esd_, $@)" ] ; then \
		echo Add EMEV eSD BOOT Option ; \
		echo "#define CONFIG_EMXX_ESDBOOT" > $(obj)include/config.h; \
	else \
	...
	@if  [ "$(findstring _cfi_, $@)" ] ; then \
		...
		echo "#define CONFIG_EMXX_UNUSE_AB" >> ./include/config.h ; \
	fi
	@if  [ "$(findstring _500M_, $@)" ] ; then \
		echo Add EMEV CPU 500MHz Option ; \
		echo "#define CONFIG_EMEV_CPU_500MHZ" >> ./include/config.h ; \
	fi
	@if  [ "$(findstring _line_, $@)" ] ; then \
		echo Add LINE SYSTEM Option ; \
		echo "#define CONFIG_EMXX_SDBOOT_LINE" >> ./include/config.h ; \
	fi
	@if  [ "$(findstring _pdium_, $@)" ] ; then \
		echo Add Palladium Option ; \
		echo "#define CONFIG_EMXX_PALLADIUM" >> ./include/config.h ; \
	fi
	...
	@if  [ "$(findstring _emmc_, $@)" ] ; then \
		echo "CONFIG_EMMC_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
	@if  [ "$(findstring _esd_, $@)" ] ; then \
		echo "CONFIG_ESD_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
	@if  [ "$(findstring _pdium_config, $@)" ] ; then \
		echo "CONFIG_IHEX_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
...
emgr_esd_config \
emgr_nts_sd_config \
emgr_nts_emmc_config \
emgr_nts_esd_config \
emgr_sd_line_config : unconfig
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo Add EMGR SD BOOT Option ; \
		echo "#define CONFIG_EMXX_SDBOOT" > $(obj)include/config.h; \
	elif [ "$(findstring _emmc_, $@)" ] ; then \
		echo Add EMGR eMMC BOOT Option ; \
		echo "#define CONFIG_EMXX_EMMCBOOT" > $(obj)include/config.h; \
	elif [ "$(findstring _esd_, $@)" ] ; then \
		echo Add EMGR eSD BOOT Option ; \
		echo "#define CONFIG_EMXX_ESDBOOT" > $(obj)include/config.h; \
	else \
		echo Add EMGR NOR BOOT Option ; \
		echo "#define CONFIG_EMXX_NORBOOT"  > $(obj)include/config.h; \
	fi
	@if  [ "$(findstring _nts_, $@)" ] ; then \
		echo Add EMGR UnUse Async Bridge Option ; \
		echo "#define CONFIG_EMXX_UNUSE_AB" >> ./include/config.h ; \
	fi
	@if  [ "$(findstring _line_, $@)" ] ; then \
		echo Add LINE SYSTEM Option ; \
	...
	@echo Use DDR 1Gbit ;
	@echo Use LANC SMSC9221 ;
	@$(MKCONFIG) -a emgr arm arm_cortexa9 emgr emxx emxx
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo "CONFIG_SD_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
	@if  [ "$(findstring _emmc_, $@)" ] ; then \
...
emgr_config_open \
emgr_emmc_config_open \
emgr_esd_config_open \
emgr_sd_config_open : unconfig 
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo Add EMGR SD BOOT Option ; \
		echo "#define CONFIG_EMXX_SDBOOT" > $(obj)include/config.h; \
	elif [ "$(findstring _emmc_, $@)" ] ; then \
...
emgr_config_close \
emgr_emmc_config_close \
emgr_esd_config_close \
emgr_sd_config_close :
	@$(MKCONFIG) -a emgr arm arm_cortexa9 emgr emxx emxx
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo "CONFIG_SD_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
...
emgr_config_add_d2g :
	@echo Add 2Gbit DDR Option ;
	@echo "#define CONFIG_EMGR_DDR_2GBit" >> $(obj)include/config.h ;

emgr_config_add_dm9000 :
	@echo Add DM9000B LANC Option ;
	@echo "#define CONFIG_EMGR_USE_DM9000" >> $(obj)include/config.h ;

emgr_config_add_vpm :
	@echo Add EMGR Use Venus Power Management IC Option ;
	@echo "#define CONFIG_EMGR_USE_VPMIC" >> $(obj)include/config.h ;

emgr_config_add_nts :
	@echo Add EMGR UnUse Async Bridge Option ;
	@echo "#define CONFIG_EMXX_UNUSE_AB" >> $(obj)include/config.h ;

emgr_help :
	@echo \ ;
	@echo -- if use auto configration ;
	@echo \ \ EM\/GR SIP \(DDR1GB\) \ \ \ \ -\> \"make emgr_config\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_emmc_config\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_esd_config\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_sd_config\" ;
	@echo \ ;
	@echo \ \ Unuse Async Bridge \ \ \ \ -\> \"make emgr_nts_emmc_config\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_nts_esd_config\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_nts_sd_config\" ;
	@echo \ ;
	@echo \ ;
	@echo -- if use manual configration, \"start\" -\> \"add\" -\> \"end\" ;
	@echo \ \ \"start\" configration \ \ -\> \"make emgr_config_open\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_emmc_config_open\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_esd_config_open\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_sd_config_open\" ;
	@echo \ ;
	@echo \ \ \"end\" configration \ \ \ \ -\> \"make emgr_config_close\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_emmc_config_close\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_esd_config_close\" ;
	@echo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \"make emgr_sd_config_close\" ;
	@echo \ ;
	@echo \ \ \"add\" optional configration ;
	@echo \ \ \ \ use DDR 2GB \ \ \ \ \ \ \ \ \ -\> \"make emgr_config_add_d2g\" ;
	@echo \ \ \ \ use LANC DM9000 \ \ \ \ \ -\> \"make emgr_config_add_dm9000\" ;
	@echo \ \ \ \ use PWC VPMIC \ \ \ \ \ \ \ -\> \"make emgr_config_add_vpm\" ;
	@echo \ \ \ \ unuse Async Bridge \ \ -\> \"make emgr_config_add_nts\" ;
	@echo \ ;
...
em1_emmc_config \
em1_sd_config \
em1_pll3_238M_config \
em1_pll3_238M_emmc_config \
em1_pll3_238M_sd_config \
em1_dkit_config \
em1_dkit_emmc_config \
em1_dkit_sd_config \
em1_dkit_sd_line_config : unconfig 
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo Add EM1 SD BOOT Option ; \
		echo "#define CONFIG_EMXX_SDBOOT" > $(obj)include/config.h; \
	elif [ "$(findstring _emmc_, $@)" ] ; then \
		echo Add EM1 eMMC BOOT Option ; \
		echo "#define CONFIG_EMXX_EMMCBOOT" > $(obj)include/config.h; \
	else \
		echo Add EM1 NOR BOOT Option ; \
		echo "#define CONFIG_EMXX_NORBOOT"  > $(obj)include/config.h; \
	fi
	@if  [ "$(findstring _dkit_, $@)" ] ; then \
		echo Add Design Kit Board Option ; \
		echo "#define CONFIG_EM1_BOARD_DKIT" >> $(obj)include/config.h; \
		echo Add EM1S Chip Option ; \
		echo "#define CONFIG_EM1_CHIP_EM1S" >> $(obj)include/config.h; \
		echo Add DDR 1Gbit Option ; \
		echo "#define CONFIG_EM1_DDR_1GBit" >> $(obj)include/config.h; \
	fi
	@if  [ "$(findstring _pll3_238M_, $@)" ] ; then \
		@echo Add EM1 PLL3 238MHz Option ; \
		@echo "#define CONFIG_EM1_PLL3_238MHZ" >> ./include/config.h ; \
	fi
	@if  [ "$(findstring _line_, $@)" ] ; then \
		echo Add LINE SYSTEM Option ; \
		echo "#define CONFIG_EMXX_SDBOOT_LINE" >> ./include/config.h ; \
	fi
	@$(MKCONFIG) -a em1 arm arm1136 em1 emxx emxx
	@if  [ "$(findstring _sd_, $@)" ] ; then \
		echo "CONFIG_SD_U_BOOT = y" >>  $(obj)include/config.mk ; \
	fi
	@if  [ "$(findstring _emmc_, $@)" ] ; then \
...
em1_cpubd_config : unconfig 
	@echo Add EM1 SD BOOT Option ;
	@echo "#define CONFIG_EMXX_SDBOOT" > ./include/config.h ;
	@echo Add Design Kit Board Option ;
	@echo "#define CONFIG_EM1_BOARD_DKIT" >> ./include/config.h ;
	@echo Add CPU Board Only Option ;
	@echo "#define CONFIG_EM1_CPU_BOARD_ONLY" >> ./include/config.h ;
	@echo Add EM1S Chip Option ;
	@echo "#define CONFIG_EM1_CHIP_EM1S" >> ./include/config.h ;
	@echo Add DDR 1Gbit Option ;
	@echo "#define CONFIG_EM1_DDR_1GBit" >> ./include/config.h ;
#	@echo Add LINE SYSTEM Option ;
#	@echo "#define CONFIG_EMXX_SDBOOT_LINE" >> ./include/config.h ;

	@$(MKCONFIG) -a em1 arm arm1136 em1 emxx emxx
	@echo "CONFIG_SD_U_BOOT = y" >>  ./include/config.mk ;
...
clean:
	...
	@if [ -d board/$(BOARDDIR)/mini-boot/mmc/ ]; then $(MAKE) -C board/$(BOARDDIR)/mini-boot/mmc/ clean ; fi
