...
int board_init (void)
{
	...
	if((config & SMU_CHIP_REVISION_MASK) != SMU_CHIP_REVISION_ES11) {
		sp0_init();
		pwcreg_init();
		sp0_deinit();
	}
...
{
#if 0
	sp0_write(PWC_R54_ADD, PWC_R54_VAL);		/* PWC R54 -> LDO5: 3.10v */
...
int checkboard(void)
{
	...
	case SMU_CHIP_REVISION_ES2:
		printf ("Board: EM/EV0 ES2 (Rev.%08x)\n", config);
		break;
	case SMU_CHIP_REVISION_ES11:
		printf ("Board: EM/EV0 ES1.1(SIP) (Rev.%08x)\n", config);
		break;
	default:
		printf ("Board: EM/EV0 ES1 (Rev.%08x)\n", config);
		break;
	}
