...
#else
static const hw_if_st *hw_if  = &sdio_if;
#endif
static volatile int *acc_mode = (volatile int *)ACC_MODE_ADDR;
static volatile int *RCA_VAL  = (volatile int *)RCA_VAL_ADDR;
...
mmc_flash_init( void )
{
	...
	MBRecord	*mbr_data = (MBRecord *)ROMBOOT_WORK_START;
	int		resp[4];
	int		c_size_n;
...
mmc_print_part( unsigned long part )
{
	...
	if (part) {
		...
		e_blk = s_blk + p_size;
		printf("  Partition %d : %08x - %08x [block]\n", part+1, s_blk, e_blk-1);
...
//	printf("\nWrite: start %08x[block] length %08x[block]\n", address, (size / MMC_BLOCKLEN_VAL));
	if (size == 0) {
		return -1;
	}
...
//	printf("\nErase: start %08x[block] length %08x[block]\n", address, (size / MMC_BLOCKLEN_VAL));
	if( size == 0 ) {
		return -1;
	}
	...
	if (fat32_flag){
		...
		while (1) {
			...
			for (j = 0; j < 16 * fat_info->sec_per_clus; j++) {
				rde = (RDEntry *)(rde_clus_data + (32 * j));
				if (!memcmp(rde->name, match_name[type], 8) || !memcmp(rde->name, MATCH_NAME[type], 8)) {
					if (type != LOAD_TYPE_UBOOT) {
						fat_info->data_num_clus = rde->cluster | ((unsigned int)rde->clusterHighWord << 16);
						return 0;
					} else {
...
mini_boot( void )
{
...
#else
	fat_info_t	fat_info;
	unsigned char	*read_data = (unsigned char *)(ROMBOOT_WORK_START + MMC_BLOCKLEN_VAL);
	unsigned char	*fat_info_data = (unsigned char *)ROMBOOT_WORK_START;
#endif

	uart_init();

#if defined(CONFIG_EMXX_SDBOOT) || defined(CONFIG_EMXX_ESDBOOT)
	/* change normal speed mode */
	hw_if->set_clock(MMC_CLK_NORMAL);
	...
	*RCA_VAL  = 1;
...
#else
	/* Get FAT infomation form SRAM */
	memcpy(&fat_info, fat_info_data, sizeof(fat_info_t));
	if (fat_info.root_entry == 0) {
		fat32_flag = 1;
	} else {
		fat32_flag = 0;
	}
	...
	/* cramfs */
	fat_info.now_fat_sec = -1;
	ret = fat_get_cluster(read_data, &fat_info, LOAD_TYPE_FS);
	if (ret != 0) {
	...
	fat_info.now_fat_sec = -1;
	ret = fat_cpy_data(read_data, &fat_info, (unsigned int)FS_START, FS_SIZE);
	if (ret != 0) {
		dbg_print("\ncopy cramfs error\n");
		goto READ_SKIP;
	}
