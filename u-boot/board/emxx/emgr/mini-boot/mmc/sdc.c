...
sdc_set_clock(unsigned int clock)
{
	if (clock == MMC_CLK_HIGH) {
		/* internal loop */
		outl(0x0100, SDM_USER);
		outl(0x0000, SDM_USER2);

		outl(SDM_CLK_HIGH_SPEED, SDM_CLK_CTRL);
	} else if (clock == MMC_CLK_NORMAL) {
		outl(SDM_CLK_NORMAL_SPEED, SDM_CLK_CTRL);
	} else {
		outl(0x0020, SDM_CLK_CTRL);	//Clk Disable
	}
}

#endif	/* defined(CONFIG_EMXX_SDBOOT) */
