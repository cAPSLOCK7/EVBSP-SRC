...
void
dm9000_check_mac_addr(const u8 *addr)
{
	int i, oft, lnk;
	u16 bi_enetaddr[6] = { 0, 0, 0, 0, 0, 0 };
	...
	for (i = 0; i < 6; i++)
		bi_enetaddr[i] = read_srom_word(i);
#endif
	...
	if (is_zero_ether_addr(bi_enetaddr) ||
	    is_multicast_ether_addr(bi_enetaddr)) {
		/* setting from environment */
		for (i = 0; i < 6; ++i) {
			bi_enetaddr[i] = addr[i];
		}
